...
const shellCommands = /(?<!([^\s,(}]{1}|\/\/.*))(echo|ls|cd|rm|cp|mv|mkdir|dd|fdisk|chmod|mount|umount|mkfs|mk2fs|mkfs.ext4|mkfs.ntfs|mkfs.ext3|mkfs.vfat)(?![\w\d]|[^\s)\{\(;]{1})/g;
    ...
    for (var i = 0; i < codeContainerList.length; i++) {
        ...
        let dropMenuContainer = createDropMenu((dropMenuContainer) => {
            ...
            for (var j = 0; j < codeSnippetList.length; j++) {
                ...
                createSnippetSelector(i + '', snippetLang + i + "" + j, (inputSnippetSelector, lblSnippetSelector) => {
...
function collectTextCodes(codeViewContents) {
    ...
    text = text.replace(/<span class="(class-name|digit|annotation|comment)">/g, "");
    return text;
}
